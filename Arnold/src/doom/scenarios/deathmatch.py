...
def _resolve_reload_path(params):
    """
    Resolve reload path robustly.

    The original code always did:
        os.path.join(params.dump_path, params.reload)
    which breaks when params.reload is already a valid path (e.g. "dumped/default/...").

    New behavior:
      1) If params.reload exists as given (relative to CWD or absolute), use it.
      2) Otherwise, interpret it relative to params.dump_path.
    """
    if not params.reload:
        return None

    # Use as-given if it's already a valid file path
    if os.path.isfile(params.reload):
        return params.reload

    # Otherwise fall back to dump_path-relative
    candidate = os.path.normpath(os.path.join(params.dump_path, params.reload))
    if os.path.isfile(candidate):
        return candidate

    # Nothing worked; return candidate so error message is informative
    return candidate
...
def main(parser, args, parameter_server=None):
    ...
    set_num_threads(1)
    # macOS / CPU-only torch builds have no CUDA; guard the call
    if params.gpu_id is not None and params.gpu_id >= 0 and torch.cuda.is_available():
    ...
    if params.reload:
        model_path = _resolve_reload_path(params)
